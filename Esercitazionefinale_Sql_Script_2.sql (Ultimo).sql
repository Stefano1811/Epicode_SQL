...
SELECT PR.categoria, sum(VE.quantità) Tot_Quantità_Richieste
FROM vendite VE
LEFT JOIN prodotti PR ON VE.idprodotto = PR.idprodotto
GROUP BY 1 
HAVING sum(quantità) = 
(SELECT max(sub.totquantità)
FROM
(SELECT P.categoria,sum(V.quantità) totquantità 
FROM vendite V
LEFT JOIN prodotti P ON V.idprodotto = P.idprodotto
GROUP BY 1) sub)
;
